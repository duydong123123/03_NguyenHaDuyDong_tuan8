import {
  ...
  const DetailUser = ({ route, navigation }) => {
    ...
    // xoá job trong mảng todos
    const deleteJob = async (id) => {
      try {
        const response = await fetch(
          `https://6541a6a8f0b8287df1fe9974.mockapi.io/user/${user.id}`,
          {
            method: "PUT",
            headers: {
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: "white" }}>
        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
          <TouchableOpacity
            style={{ marginLeft: 10 }}
            // click sẽ quay lại màn hình trước đó
          ...
          renderItem={({ item }) => (
            <View
              style={{
                ...
                marginHorizontal: 10,
                marginVertical: 5,
                backgroundColor: "rgba(222, 225, 250, 0.20)",
              }}
            >
              ...
              >
                <TouchableOpacity
                  // click sẽ delete job và quay lại màn hình trước đó
                  onPress={() => {
                    deleteJob(item.id);
                    navigation.goBack();
                  }}
